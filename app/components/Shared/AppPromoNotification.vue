<template>
  <ClientOnly>
    <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="translate-y-4 opacity-0" enter-to-class="translate-y-0 opacity-100" leave-active-class="transition duration-200 ease-in" leave-from-class="translate-y-0 opacity-100" leave-to-class="translate-y-4 opacity-0">
      <aside v-if="isVisible && promo" class="fixed inset-x-0 bottom-[calc(0.5rem+env(safe-area-inset-bottom))] z-50 mx-auto w-[calc(100%-1.5rem)] max-w-sm lg:bottom-8 lg:left-auto lg:right-8 lg:mx-0">
        <div class="relative max-h-[25vh] overflow-hidden rounded-xl border border-stone-200 bg-white p-4">
